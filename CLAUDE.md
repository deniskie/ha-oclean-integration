...
**Other record fields — analysed, intentionally NOT exposed (2026-06-16):**
- **`pressureCode` (APK `a.b.m14b` / smali `a/b.smali`)** — an argmax-run algorithm over record bytes **10-14** (gated on `byte10 < 0x55`) returning `50/60/70/80/90`. Deliberately **not** implemented: it operates on a byte range that differs from our `pressureRatio` (11-15), has **no user-facing label found** in the APK, and we already expose the raw `pressureRatio`. Reimplementing it risks a subtly-wrong derived sensor for no confirmed gain.
- **`gestureCode` (2-bit, byte 30) and `powerArray` (12× 2-bit, bytes 30-32)** — extracted into `last_brush_gesture_code` / `last_brush_power_array` as raw research fields, but **the 0-3 value meanings are not determinable** from the decompiled code (no string/enum mapping in the UI layer). Not surfaced as labelled sensors until the semantics are confirmed.
